...
class DynamicSMC(Coverage):
    ...
    def time_steps(self, n_steps, dt, return_trajectories=False):
        ...
        # Set probability distribution for the ground truth
        self.target_distribution.compute_mu_ground_truth_k()

        mu_ground_truth_k = self.target_distribution.get_mu_ground_truth_k()
        ...
        for i_step in range(n_steps):
            ...
            #### Collect the ergodic measure of the ground truth

            #Compute time-average coverage explicitly
            c_k_avg = self.C_k

            S_k_gt_avg = mu_ground_truth_k* n_agents * self.current_time * dt - c_k_avg

            # Spatial reconstruction of the error field
            S_k_gt_avg = S_k_gt_avg.real

            #self._plot_spectral_error(S_k_avg, Lx, Ly)

            self.S_k_gt_avg.append(np.linalg.norm(S_k_gt_avg)) # save norm

            #### Also collect the ergodic measure of the target distribution
            S_k_avg = mu_k * n_agents * self.current_time * dt - c_k_avg
            S_k_avg = S_k_avg.real
            self.S_k_avg.append(np.linalg.norm(S_k_avg))
            ...
            for iagent, agent in enumerate(self.agents):
                ...
                Bjx, Bjy = self.compute_Bjvec_from_Sk(S_k, agent, rho=0.05)
