...
class ProbDist:
    ...
    def __init__(self, xmin=0.0, xmax=1.0, ymin=0.0, ymax=1.0, Nx=235, Ny=450):
        ...
        #######################################################
        # numpy array for ground truth probability distribution
        self.mu_gt = np.ones((self.Nx, self.Ny))
        # numpy array for fourier coefficients of ground truth probability distribution
        self.mu_gt_k = np.zeros((self.Nx, self.Ny))
        # boolean that indicates whether fourier coefficients of
        # ground truth probability distribution are current
        self.mu_gt_k_current = False
    ...
    def get_mu_ground_truth_k(self):
        """ Returns Fourier coefficients of probability distribution """
        return self.mu_gt_k


    def compute_mu_ground_truth_k(self):
        """" Computes Fourier coefficients of probability distribution """

        if self.mu_gt_k_current:
            return

        # first normalize target probability distribution
        """ Normalizes target probability distribution """

        integral = np.sum(self.mu_gt) * self.dx * self.dy

        self.mu_gt = self.mu_gt / integral

        #self.mu_k = fft2(self.mu)
        self.mu_gt_k = (dct(dct(self.mu_gt.T, norm=None).T / 2.0, norm=None) / 2.0) * self.dx * self.dy

        self.mu_gt_k_current = True
    ...
    def set_prob_dist_ground_thruth(self, array):
        """ Sets the probability distribution from given array """
